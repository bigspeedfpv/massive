...
#[aliases("pardon")]
#[required_permissions(BAN_MEMBERS)]
async fn whitelist(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    let id: UserId;
    ...
    if let Some(current_guild) = msg.guild(ctx).await {
        let ban_result = current_guild.unban(ctx, id).await;

        if let Ok(_) = ban_result {
            Ok(())
        } else {
            Err(From::from(
                "Failed to unban user. Do I have the correct permissions?",
            ))
        }
    } else {
        Err(From::from(format!(
