...
async fn settings(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    let response = msg.channel_id.send_message(ctx, |m| m.embed(|e| e
        .title(&format!("Settings for {}", server_name))
        .color(0xB330FF)
        .description(&format!("
            **Guild Prefix** (prefix): {}
...
#[command]
#[description("Changes a setting in the current guild. Setting names are specified in parentheses.")]
#[usage("<setting> [value]")]
async fn set(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    if args.is_empty() { return Err(From::from("You must provide a setting to change!")) }
    ...
    if let Ok(value) = args.single::<String>() {
        ...
        let help = match &setting[..] {
            ...
            _ => { return Err(From::from("Unrecognized setting! Please run `settings` to view available options.")) }
        };

        let prompt = msg.channel_id.send_message(ctx, |m| m.embed(|e| {
            e.title(&format!("Please enter the value for {}.", setting))
             .description(help)
             .color(0xB330FF)
        })).await;

        if let Some(prompt_res) = msg.channel_id
            .await_reply(&ctx)
            ...
            .await 
        {
            ...
            if let Ok(prompt_msg) = prompt { let _ = prompt_msg.delete(ctx).await; }
